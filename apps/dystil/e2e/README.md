...
```powershell
# 1. Install frontend dependencies (from repo root or apps/dystil)
cd apps/dystil
bun install
...
**Binary not found**
```
Error: Dystil debug binary not found at …\src-tauri\target\debug\dystil-app.exe
```
...
Check `apps/dystil/.e2e/` for log files after a run. The app launcher pipes stdout/stderr with an `[app]` prefix to the test runner console.
