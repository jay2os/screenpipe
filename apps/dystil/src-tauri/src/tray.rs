...
fn prefetch_tray_menu_data(app: &AppHandle) -> TrayMenuData {
    ...
    let is_logged_in = settings.user.token.as_ref().map_or(false, |token| !token.is_empty())
        ...
        if let Err(e) = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            crate::window::with_autorelease_pool(|| {
                ...
                let icon = match app.path().resolve(
                    "assets/dystil-logo-tray-white.png",
                    tauri::path::BaseDirectory::Resource,
                ) {
                    Ok(path) => tauri::image::Image::from_path(path).ok(),
                    Err(_) => {
                        tauri::image::Image::from_path("assets/dystil-logo-tray-white.png").ok()
                    }
                };
...
fn create_dynamic_menu(
    ...
    if !data.onboarding_completed && !data.app_ui_hidden {
        menu_builder = menu_builder
            ...
            .item(&PredefinedMenuItem::separator(app)?)
            .item(&MenuItemBuilder::with_id("quit", "Quit dystil").build(app)?);
    ...
    if !data.app_ui_hidden {
        menu_builder = menu_builder
            .item(&MenuItemBuilder::with_id("open_app", "Open dystil").build(app)?)
            .item(&PredefinedMenuItem::separator(app)?);
    }
    ...
    let version_text = match (is_beta, is_enterprise) {
        (_, true) => format!("dystil v{} (Enterprise)", app.package_info().version),
        (true, false) => format!("dystil v{} (Beta)", app.package_info().version),
        (false, false) => format!("dystil v{}", app.package_info().version),
    };
    menu_builder = menu_builder.item(
    ...
    menu_builder = menu_builder.item(
        &MenuItemBuilder::with_id("quit", "Quit dystil")
