...
async fn fetch_app_activity(api: &LocalApiContext) -> Result<Vec<AppActivity>, String> {
    ...
    let resp = api
        ...
        .json(&serde_json::json!({
            "query": "SELECT app_name, COUNT(*) as cnt FROM frames WHERE datetime(timestamp) > datetime('now', '-30 minutes') AND app_name != '' AND app_name != 'screenpipe' AND app_name != 'dystil-app' GROUP BY app_name ORDER BY cnt DESC LIMIT 15"
        }))
...
async fn fetch_window_activity(api: &LocalApiContext) -> Result<Vec<WindowActivity>, String> {
    ...
    let resp = api
        .apply_auth(client.post(api.url("/raw_sql")))
        .json(&serde_json::json!({
            "query": "SELECT app_name, window_name, COUNT(*) as cnt FROM frames WHERE datetime(timestamp) > datetime('now', '-30 minutes') AND app_name != '' AND app_name != 'screenpipe' AND app_name != 'dystil-app' AND window_name != '' GROUP BY app_name, window_name ORDER BY cnt DESC LIMIT 20"
        }))
        .timeout(std::time::Duration::from_secs(10))
        .send()
        .await
...
mod tests {
    ...
    // ─── Benchmark tests ─────────────────────────────────────────────────────
    // Run with: cargo test -p dystil-app -- --ignored benchmark --nocapture
