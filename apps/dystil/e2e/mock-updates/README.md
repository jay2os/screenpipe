...
Set `apps/dystil/src-tauri/Cargo.toml` to a newer semver (example: `2.4.229`), then run:
...
In another terminal, set `apps/dystil/src-tauri/Cargo.toml` to an older semver (must be lower than step 1), then run:
...
- Scripts used above are in `apps/dystil/package.json`:
  - `updater-local:build`
