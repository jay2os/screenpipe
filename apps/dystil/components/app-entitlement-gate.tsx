...
export function AppEntitlementGate({ children }: { children: React.ReactNode }) {
  ...
  const devLoginBlock = isDevLoginEnabled() ? (
    <div className="mt-5 border-t border-border pt-4">
      ...
      <input
        ...
        placeholder="paste token or dystil://…api_key=…"
        spellCheck={false}
  ...
  if (!isSettingsLoaded) {
    return (
      <EntitlementShell
        title="loading"
        description="checking local settings before starting dystil."
      >
  ...
  if (!user?.token) {
    return (
      <EntitlementShell
        ...
        description="Dystil now needs an account with an active plan before recording starts."
      >
