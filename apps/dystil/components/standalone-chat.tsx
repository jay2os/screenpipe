...
// Sedimentation loader — four layered strokes with a single orange highlight
// that cycles bottom-to-top in an infinite loop.
function SedimentationLoader({
  phase = "analyzing",
  label,
  toolName,
  thinkingSecs,
  ...
  useEffect(() => {
    const interval = phase === "streaming" ? 400 : 300;
    const id = window.setInterval(() => {
      setActiveLayer((prev) => (prev - 1 + 4) % 4);
    }, interval);
  ...
  const WHITE = "url(#sed-white)";
  const ORANGE = "url(#sed-orange)";
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      >
        <defs>
          <linearGradient id="sed-white" x1="120" y1="96" x2="392" y2="420" gradientUnits="userSpaceOnUse">
            <stop offset="0" stopColor="#FFFFFF" />
            <stop offset="0.58" stopColor="#F4F4F4" />
            <stop offset="1" stopColor="#D8D8D8" />
          </linearGradient>
          <linearGradient id="sed-orange" x1="126" y1="200" x2="386" y2="292" gradientUnits="userSpaceOnUse">
            <stop offset="0" stopColor="#FF6A1A" />
            <stop offset="1" stopColor="#EF4B00" />
          </linearGradient>
        </defs>
        <g strokeLinecap="round" strokeLinejoin="round" fill="none">
          {/* 0 — top diamond */}
          <path
            d="M122 164 L256 92 L390 164 L256 236 Z"
            stroke={activeLayer === 0 ? ORANGE : WHITE}
            strokeWidth="18"
          />
          {/* 1 — upper wave */}
          <path
            d="M120 222 C150 240 185 260 216 278 C240 292 272 292 296 278 C327 260 362 240 392 222"
            stroke={activeLayer === 1 ? ORANGE : WHITE}
            strokeWidth="18"
          />
          {/* 2 — mid wave */}
          <path
            d="M120 282 C150 300 185 320 216 338 C240 352 272 352 296 338 C327 320 362 300 392 282"
            stroke={activeLayer === 2 ? ORANGE : WHITE}
            strokeWidth="18"
          />
          {/* 3 — bottom wave */}
          <path
            d="M132 338 C160 356 192 376 220 392 C242 405 270 405 292 392 C320 376 352 356 380 338"
            stroke={activeLayer === 3 ? ORANGE : WHITE}
            strokeWidth="18"
          />
        </g>
      </svg>
...
// existing assets in apps/dystil/public/images/.
...
export function StandaloneChat({
  ...
  return (
    <div ref={dropRootRef} className={cn("flex flex-col bg-background", className ?? "h-screen")} data-testid="section-home">
      ...
      <div className={cn("flex-1 flex min-h-0", messages.length === 0 && !activePipeExecution && "flex-col justify-center items-center")} data-browser-panel-host>
      <div className={cn("flex-1 flex flex-col min-w-0", messages.length === 0 && !activePipeExecution && "flex-none")}>
    <div className={cn("flex overflow-hidden", messages.length === 0 && !activePipeExecution ? "justify-center overflow-visible" : "flex-1")}>
        <AnimatePresence>
        ...
        <div
          ...
          className={cn("relative min-w-0 overflow-x-hidden", messages.length === 0 && !activePipeExecution ? "flex flex-col items-center justify-center overflow-visible" : "flex-1 overflow-y-auto")}
      ...
      {/* Input */}
      <div ref={inputSectionRef} className={cn("relative bg-gradient-to-t from-muted/20 to-transparent", messages.length === 0 && !activePipeExecution ? "" : "border-t border-border/50")}>
