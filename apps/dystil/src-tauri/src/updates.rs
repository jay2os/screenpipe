            ...
            *self.pending_update.lock().await = Some(PendingUpdateSnapshot {
...

            {
                ...
                let icon_path = if theme == Mode::Light {
                    "assets/dystil-logo-tray-updates-black.png"
                } else {
                    "assets/dystil-logo-tray-updates-white.png"
                };
            ...
            if let Some(ref item) = self.update_menu_item {
                ...
                item.set_text("Downloading latest version of dystil")?;
            }
                    ...
                    *self.pending_update.lock().await = None;
                    ...
                    std::thread::spawn(move || {
                        let _ = app_notif
                            .notification()
                            .builder()
                            .title("dystil update failed")
                            .body(format!(
                                "v{} couldn't download — open dystil to retry",
                                version_str
                            ))
                            .show();
                    });
...
fn check_whats_new(app: &tauri::AppHandle) {
    ...
    tokio::spawn(async move {
        ...
        let body = if release_notes.is_empty() {
            format!(
                "dystil updated to **v{}**! check the [changelog](https://screenpi.pe/changelog) for details.",
                current_version
            )
        } else {
        ...
        match client
            ...
            .json(&serde_json::json!({
                ...
                "type": "app-update",
                "title": format!("🎉 dystil v{} — what's new", current_version),
                "body": body,
            }))
            .send()
            .await
