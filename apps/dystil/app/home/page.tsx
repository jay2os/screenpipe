  ...
  const renderMainSection = () => {
    switch (activeSection) {
      case "home":
        ...
        // and Pi event listeners survive navigation.
        return null;
      case "history":
        return (
          <ChatHistoryView
          ...
          <AppSidebar collapsed={sidebarCollapsed} className="pl-4">
            ...
            <div className={cn(isTranslucent ? "vibrant-sidebar-border" : "", "border-b", sidebarCollapsed ? "px-2 py-3" : "px-4 py-3")}>
              ...
              <div className={cn("flex items-center", sidebarCollapsed ? "justify-center" : "justify-between")}>
                {!sidebarCollapsed && <div className="flex items-center gap-1.5"><PipeAIIcon size={30} animated={false} /><h1 className={cn("text-lg font-bold tracking-[0.2em] uppercase", isTranslucent ? "vibrant-heading" : "text-foreground")}><span>D</span><span style={{ color: "#C02E31" }}>Y</span><span>STIL</span></h1></div>}
              </div>
            </div>
            )}
              ...
              {!sidebarCollapsed ? (
                <div
                  className={cn(
                    ...
                    // bottom nav.
                    "flex-1 min-h-0 flex flex-col mt-2 -mx-2 pt-2 pb-6",
                ...
                {(() => {
                  const btn = (
                    ...
                    >
                      ...
                      {!sidebarCollapsed && <span className="font-medium text-xs truncate">History</span>}
                    </button>
                  );
                  if (sidebarCollapsed) {
                    return (
