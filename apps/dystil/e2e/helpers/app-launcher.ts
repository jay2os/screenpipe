...
export function getAppPath(): string {
  ...
  const name = process.platform === 'win32' ? 'dystil-app.exe' : 'dystil-app';
...
export async function startApp(port = WEBDRIVER_PORT): Promise<ReturnType<typeof spawn> | null> {
  ...
  if (!existsSync(appPath)) {
    throw new Error(
      `Dystil debug binary not found at ${appPath}. Build with e2e feature: cd apps/dystil && bun tauri build --no-sign --debug --verbose --no-bundle -- --features e2e`
    );
  }
